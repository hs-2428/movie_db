...
class ReleaseNotificationWorkerTest {
    ...
    fun `worker handles movies with invalid date formats gracefully`() = runTest {
        ...
        val moviesWithMixedDates = listOf(
            ...
            createMovieEntity(4, "Null Date Movie", "2024-13-45") // Invalid date
        )
    ...
    private fun createWorker(): ReleaseNotificationWorker {
        return TestListenableWorkerBuilder<ReleaseNotificationWorker>(context)
            .build() as ReleaseNotificationWorker
    }
    ...
    // Custom worker builder that injects our mock
    private fun TestListenableWorkerBuilder<ReleaseNotificationWorker>.build(): ReleaseNotificationWorker {
        val workerParams = mockk<WorkerParameters>(relaxed = true)
        return ReleaseNotificationWorker(context, workerParams, mockMovieDao)
    }
}